...
const STORAGE_KEY_TOKEN = 'bolaoCopa_token';
...
function renderizarLogin() {
  ...
  app.innerHTML = `
    <div class="tela-login">
      ...
      <p class="legenda">Toque na sua foto, digite seu PIN de 4 dígitos e comece a palpitar.</p>
      <div class="grade-participantes">
        ${estado.participantes.map(p => `
          <button class="cartao-participante" data-id="${p.id}" onclick="abrirPinModal('${p.id}')">
    ...
    <div id="pin-modal-wrap"></div>
  `;
}

function abrirPinModal(participanteId) {
  const p = estado.participantes.find(x => x.id === participanteId);
  const wrap = document.getElementById('pin-modal-wrap');
  wrap.innerHTML = `
    <div class="overlay-pin" onclick="if(event.target===this) fecharPinModal()">
      <div class="modal-pin">
        <img src="${p.foto || fotoPadrao()}" onerror="this.src='${fotoPadrao()}'">
        <h3 style="font-size:16px; margin-top:10px;">${escapeHtml(p.nome)}</h3>
        <p class="legenda" style="margin:6px 0 16px; font-size:13px;">Digite seu PIN de 4 dígitos</p>
        <input type="password" inputmode="numeric" maxlength="4" id="pin-input" class="pin-input" autofocus
               onkeydown="if(event.key==='Enter') confirmarPin('${participanteId}')">
        <div id="pin-erro" class="pin-erro"></div>
        <div style="display:flex; gap:8px; margin-top:14px;">
          <button class="btn-secundario" style="flex:1;" onclick="fecharPinModal()">Cancelar</button>
          <button class="btn-primario" style="flex:1;" onclick="confirmarPin('${participanteId}')">Entrar</button>
        </div>
      </div>
    </div>
  `;
...
function fecharPinModal() {
  document.getElementById('pin-modal-wrap').innerHTML = '';
}

async function confirmarPin(participanteId) {
  const pin = document.getElementById('pin-input').value.trim();
  const erroEl = document.getElementById('pin-erro');

  if (!/^\d{4}$/.test(pin)) {
    erroEl.textContent = 'O PIN precisa ter 4 números.';
    return;
  }

  try {
    const resp = await apiPost({ action: 'login', participanteId: participanteId, pin: pin });
    estado.participanteAtual = participanteId;
    estado.token = resp.token;
    localStorage.setItem(STORAGE_KEY, participanteId);
...
function renderizar() {
  ...
  app.innerHTML = `
    ...
    <div class="abas">
      ...
      <button class="aba-btn ${estado.abaAtiva === 'todos' ? 'ativa' : ''}" onclick="mudarAba('todos')">Quem já palpitou</button>
...
function renderizarAbaAtiva() {
  ...
  else if (estado.abaAtiva === 'todos') renderizarTodosPalpites();
...
// ---------- ABA QUEM JÁ PALPITOU ----------

function renderizarTodosPalpites() {
  const container = document.getElementById('conteudo-aba');
  const agora = new Date();

  if (estado.jogos.length === 0) {
    container.innerHTML = '<div class="vazio">Nenhum jogo cadastrado ainda.</div>';
    return;
  }

  container.innerHTML = `
    <div class="lista-jogos">
      ${estado.jogos.map(jogo => renderizarCartaoTodos(jogo, agora)).join('')}
    </div>
  `;
}

function renderizarCartaoTodos(jogo, agora) {
  const inicio = new Date(jogo.dataHora);
  const travado = agora >= inicio; // travado = jogo já começou ou já tem resultado
  const dataFormatada = inicio.toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });

  const palpitesDoJogo = estado.palpites.filter(p => p.jogoId === jogo.id);
  ...
  const linhasParticipantes = estado.participantes.map(part => {
    const palpite = palpitesDoJogo.find(p => p.participanteId === part.id);

    let valorExibido;
    if (!travado) {
      // Jogo ainda aberto: não revela o placar, só se já palpitou
      valorExibido = palpite
        ? '<span class="check-palpitou">✓ Palpitou</span>'
        : '<span class="check-faltou">— Ainda não</span>';
    } else if (!palpite) {
      valorExibido = '<span class="check-faltou">Não palpitou</span>';
    } else {
      valorExibido = `<span class="placar-revelado">${palpite.placarCasa} x ${palpite.placarFora}</span>`;
      if (jogo.finalizado) {
        const pts = calcularPontosJogo(jogo, palpite);
        valorExibido += ` <span class="resultado-palpite-tag pts-${pts}" style="margin-left:6px;">+${pts}</span>`;
      }
    }

    return `
      <div class="linha-palpite-todos">
        <img src="${part.foto || fotoPadrao()}" onerror="this.src='${fotoPadrao()}'">
        <span class="nome-palpite-todos">${escapeHtml(part.nome)}</span>
        <span class="valor-palpite-todos">${valorExibido}</span>
      </div>
    `;
  }).join('');

  return `
    <div class="cartao-jogo ${travado ? 'travado' : ''} ${jogo.finalizado ? 'finalizado' : ''}">
      <div class="linha-meta">
        <span class="tag-fase">${escapeHtml(jogo.fase || 'Fase')} · ${dataFormatada}</span>
        <span class="tag-status ${statusClasse}">${statusTag}</span>
      </div>
      <div class="confronto" style="margin-bottom:14px;">
        <div class="time"><span class="nome-time">${escapeHtml(jogo.timeCasa)}</span></div>
        <div class="versus">${jogo.finalizado ? jogo.placarCasaReal + ' x ' + jogo.placarForaReal : 'x'}</div>
        <div class="time"><span class="nome-time">${escapeHtml(jogo.timeFora)}</span></div>
      </div>
      <div class="grade-todos-palpites">
        ${linhasParticipantes}
      </div>
    </div>
  `;
